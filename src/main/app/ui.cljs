(ns app.ui
  (:require
   ...
   [com.fulcrologic.fulcro.dom :as dom :refer [h5 h4 div p li ul button]]
   [app.mutations :as api]))


(defsc Person [this {:person/keys [person/name person/age]} {:keys [onDelete]}]
  {:query [:person/name :person/age]
   :initial-state (fn [{:keys [name age] :as params}] {:person/name name :person/age age})}
  (li
   (h5 (str name " (age " age ")") (button {:onClick #(onDelete name)} "X"))))
